import React from 'react'

export default function Feature() {
    return (
        <section >
            <div className='flex justify-center py-5'>
                <h1 className='font-semibold text-xl justify-start'>Feature works</h1>
                </div>
            <div className="font-custom max-w-3xl mx-auto bg-white rounded-lg p-4  overflow-hidden flex flex-col md:flex-row">
            

            <div className="md:w-1/2">
                <img class="w-80   " src="/images/Rectangle 30.png" alt="Feature Post Image" />
            </div>


            <div className="p-4  flex flex-col justify-center">

                


                <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-3">Feature Post Title</h2>
                
                <div className="flex items-center text-gray-500 text-sm mb-3">
                    <span>October 25, 2024</span>
                    <span className="mx-2 border-l border-gray-300 h-4"></span>
                    <span>Tech</span>
                </div>


                <p class="text-gray-600 text-base mb-6">
                    This is a brief introduction to the feature post. It should be engaging and give the reader an overview of what the post will cover.
                </p>
                 <span className="mx-3 border-b border-gray-300 h-4"></span>

            </div>
           
        </div>
        </section>
        

    )
}
